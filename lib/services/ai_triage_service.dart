...
class AiTriageService {
  ...
  Future<TriageResult> triageEmergency({
    ...
    if (skipCloud) {
      appLog.d(
        '[Triage] Connectivity=none — skipping Tier 1 cloud (saves 3s timeout)',
      );
    } else {
      // Hard-fallback budget from the rulebook: move to the next tier within 3s.
    ...
    try {
      ...
      return await _buildTier3Triage(
        transcript: audioTranscript,
        location: locationString,
        severityHint: accelerometerSeverityHint,
        languageCode: languageCode,
      ...
      appLog.w(
        '[Triage] Tier 3 failed — using Tier 4 keyword classifier',
        error: e,
        stackTrace: st,
      );
    }
