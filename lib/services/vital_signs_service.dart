...
class VitalSignsLogger extends StateNotifier<VitalSigns?> {
  ...
  String _interpretForDispatch({
    ...
    final flags = <String>[];
    if (bpm >= 120) {
      flags.add('tachycardia (very fast pulse)');
    }
    if (bpm <= 45) {
      flags.add('bradycardia (very slow pulse)');
    }
    if (respiratoryRate >= 24) {
      flags.add('tachypnoea (rapid breathing)');
    }
    if (respiratoryRate <= 8) {
      flags.add('bradypnoea (slow breathing)');
    }
    if (bloodOxygen < 90) {
      flags.add('hypoxia (SpO2 <90% — CRITICAL)');
    }
