...
class FamilyTrackingService {
  ...
  Future<({bool ok, String detail})> registerAndNotifyContact({
    ...
    if (contacts.isEmpty) {
      return (
        ok: true,
        detail: 'Family link active — add emergency contacts in Medical profile to auto-SMS.',
      );
    }
    ...
    if (!kIsWeb && Platform.isAndroid) {
      final results = await Future.wait(contacts.map((c) async {
        final phone = normalizePhoneDigits(c);
        if (phone == null) return false;
        return await sendSmsDirectAndroid(phone, body);
      }));
    ...
    return (
      ok: true,
      detail:
          'Family link ready — iOS cannot auto-SMS contacts; share: $trackingUrl',
    );
  }
}
