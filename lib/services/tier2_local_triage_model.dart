...
class Tier2LocalTriageModel {
  ...
  Tier2LocalTriage classify({
    ...
    if (_hasAny(t, const ['not breathing', 'no pulse', 'cardiac arrest'])) score += 8;
    if (_hasAny(t, const ['unconscious', 'passed out', 'unresponsive'])) score += 6;
    if (_hasAny(t, const ['bleeding heavily', 'severe bleeding', 'spurting'])) score += 6;
    ...
    // Moderate indicators.
    if (_hasAny(t, const ['pain', 'hurt', 'bleeding', 'crash', 'accident'])) score += 2;
  ...
  List<String> _servicesFromText(String t) {
    ...
    if (_hasAny(t, const ['police', 'hit and run', 'drunk', 'attack', 'crime'])) {
      services.add('police');
    }
  ...
  String _firstAidQueryFromText(String t) {
    if (t.contains('bleed')) return 'severe bleeding wound management tourniquet';
    if (_hasAny(t, const ['burn', 'smoke'])) return 'burn wound first aid cool water';
    if (_hasAny(t, const ['not breathing', 'cpr', 'choking'])) {
      return 'CPR rescue breathing Heimlich';
    }
    if (_hasAny(t, const ['fracture', 'broken'])) return 'fracture immobilization splint';
    if (_hasAny(t, const ['head injury', 'concussion'])) return 'head injury concussion protocol';
